...
def reduce_requests(req_sources: "List[Tuple]", sort_reduce_funcs: "List[ReduceFunction]", logger: "Callable") -> "RowIterator":
    """
    Reduces the passed request sources tuples by using the
    sort-reduce functions, yielding the first of the remaining items.
    """
    ...
    def next_result(items: "List[Sequence]") -> "RowIterator":
        result = items
        debug_log = []
        for filterfunc in sort_reduce_funcs:
            newresult = list(filterfunc(result))
            debug_log.append('filter %s removed %s' % (filterfunc.__name__, list(filterfalse(newresult.__contains__, result))))
            result = newresult

        if len(result) == 0:
            logger(logging.WARNING, 'all requests with id %d were filtered, could not prepare request', items[0][request_id_col])
    ...
    for idx in range(1, len(req_sources)):
        if cur_request_id != req_sources[idx][request_id_col]:
            yield from next_result(current_items)
