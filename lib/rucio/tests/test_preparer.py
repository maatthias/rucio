...
from rucio.core.request import sort_requests_minimum_distance, request_id_col, distance_col, dest_rse_id_col, source_rse_id_col, extra_transfertool_col, get_transfertool_filter, get_supported_transfertools
...
def test_two_sources_one_destination(db_session, vo, file, source_rse, dest_rse, mock_request):
    ...
    def teardown(rse):
        delete_replicas(rse_id=rse.rse_id, files=[file], session=rse.db_session)

    with GeneratedRSE(vo=vo, db_session=db_session, setup_func=setup, teardown_func=teardown) as source2_rse:
...
def test_filter_requests_for_transfertools():
    ...
    def get_transfertools(rse_id: str) -> "Set[str]":
        ...
        if rse_id == 'rse1':
            ...
            assert False, 'rse_id out of range'
