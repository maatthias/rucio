...
def pytest_configure(config):
    config.addinivalue_line(
        "markers", "dirty: marks test as dirty, i.e. tests are leaving structures behind"
    )
    config.addinivalue_line(
        "markers", "noparallel(reason): marks test being unable to run in parallel to other tests, i.e. changing global state"
    )
    if config.pluginmanager.hasplugin("xdist"):
        from rucio.tests.ruciopytest.xdist import NoParallelXDist
...
def pytest_xdist_make_scheduler(config, log):
    ...
    if dist == "rucio":
        from rucio.tests.ruciopytest.xdist import NoParallelAndLoadScheduling
